...
void main() {
	...
	float samples = 3.0 + ceil(sample_pixel_range * params.quality);
...
//	}
	...
	vec2 sample_uv_range = blue_uv - uv;
	vec2 uv_delta = sample_uv_range / samples;
	...
	//vec2 sample_uv = blue_uv + uv_delta * (0.5 + rando);
	float rando = nrand(uv + params.jitter_seed) - 0.5;
	vec2 base_sample_uv = uv + 0.5 * uv_delta;
	vec2 sample_uv = uv_delta * rando;

	//	vec2 sample_uv = uv + 0.5 * uv_delta + uv_delta * rando;
	...
	vec2 base_spectrum_uv = vec2(0.1 + 0.5 * spectrum_delta, 0.0);

	vec2 spectrum_uv; // = vec2(0.1 + spectrum_delta * rando + 0.5 * spectrum_delta, 0.0);
	spectrum_uv.x = base_spectrum_uv.x + spectrum_delta * rando;
	...
	filter_sum = vec3(0.0);
	sum = vec3(0.0);
	//	filter_sum = texture(spectrum_texture, spectrum_uv).rgb;
	//	sum = filter_sum * texture(copy_texture, sample_uv).rgb;
	//	vec3 new_sum = texture(spectrum_texture, spectrum_uv + 0.4).rgb;
	//	filter_sum += new_sum;
	//	sum += new_sum * texture(copy_texture, sample_uv + sample_uv_range * 0.5).rgb;
	//	new_sum = texture(spectrum_texture, spectrum_uv + 0.8).rgb;
	//	filter_sum += new_sum;
	//	sum += new_sum * texture(copy_texture, sample_uv + sample_uv_range).rgb;
	...
	for (float i = 0.0; i < samples; ++i) {
		...
		//	vec3 spectrum_filter = max(texture(spectrum_texture, spectrum_uv).rgb, 0.000001);
		rando = nrand(base_sample_uv + params.jitter_seed) - 0.5;
		//	vec3 spectrum_filter = max(texture(spectrum_texture, base_spectrum_uv + rando * spectrum_delta).rgb, 0.000001);
		vec3 spectrum_filter = texture(spectrum_texture, base_spectrum_uv + rando * spectrum_delta).rgb;

		//	spectrum_pos += spec_amount;
		// spectrum_filter /= 255.0;
		// vec2 sample_uv = spec_jit * dir_uv;
		// vec2 sample_uv = blue_uv + uv_delta * i;
		sum += texture(copy_texture, base_sample_uv + rando * uv_delta).rgb * spectrum_filter;
		filter_sum += spectrum_filter;
		sample_uv += uv_delta;
		spectrum_uv.x += spectrum_delta;
		base_spectrum_uv += spectrum_delta;
		base_sample_uv += uv_delta;
	...
	imageStore(ca_image, pos, vec4(sum / max(filter_sum, 0.00000000), 1.0));
	...
	// imageStore(ca_image, pos, texture(refraction_texture, uv));

	// imageStore(ca_image, pos, vec4(texture(refraction_texture, abs(2.0 * half_uv)).a/10.0,0.0,0.0,1.0));

//	imageStore(ca_image, pos, vec4(1.0, 0.0, 0.0, 1.0));
// imageStore(ca_image, pos, texture(spectrum_texture, vec2(0.4)));
