...
#if defined(MODE_CA_PROCESS_TWO_TONE) || defined(MODE_CA_PROCESS_THREE_TONE) || defined(MODE_CA_PROCESS_SPECTRUM_DEFAULT) || defined(MODE_CA_PROCESS_SPECTRUM_CUSTOM)
...
#ifdef MODE_CA_PROCESS_SPECTRUM_CUSTOM
...
void main() {
	...
	vec3 sum1 = vec3(texture(copy_texture, uv1).r,
			texture(copy_texture, uv1 + uv_delta1).g,
			texture(copy_texture, uv1 + 2 * uv_delta1).b);
	vec3 sum2 = vec3(texture(copy_texture, uv2).r,
			texture(copy_texture, uv2 + uv_delta2).g,
			texture(copy_texture, uv2 + 2 * uv_delta2).b);
	vec3 sum3 = vec3(texture(copy_texture, uv3).r,
			texture(copy_texture, uv3 + uv_delta3).g,
			texture(copy_texture, uv3 + 2 * uv_delta3).b);
	vec3 sum4 = vec3(texture(copy_texture, uv4).r,
			texture(copy_texture, uv4 + uv_delta4).g,
			texture(copy_texture, uv4 + 2 * uv_delta4).b);
#endif
...
#if defined(MODE_CA_PROCESS_SPECTRUM_DEFAULT) || defined(MODE_CA_PROCESS_SPECTRUM_CUSTOM)
...
#endif
#ifdef MODE_CA_PROCESS_SPECTRUM_DEFAULT
		const vec3 spectrum_filter = max(vec3(0.0), 1.0 - abs(vec3(5.0 * base_spectrum_uv_x) - vec3(1.5, 2.5, 3.5)));
#endif
#ifdef MODE_CA_PROCESS_SPECTRUM_CUSTOM
	...
	imageStore(ca_image, pos1, vec4(sum1 / filter_sum, 1.0));
	imageStore(ca_image, pos2, vec4(sum2 / filter_sum, 1.0));
	imageStore(ca_image, pos3, vec4(sum3 / filter_sum, 1.0));
	imageStore(ca_image, pos4, vec4(sum4 / filter_sum, 1.0));
#endif
